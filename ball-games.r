...
ball <- function(t, y, parms)
{
  with(as.list(c(parms, y)), {
    ...
    if(is.na(spin1))
      spin1 <- 0
    #dy2 <- coeff*(CD*(y[2] - W[1]) - CL*(vw[2]*(y[6] - W[3]) - vw[3]*(y[4] - W[2]))/w)
    dy2 <- coeff*CD*(y[2] - W[1]) - spin1
    dy3 <- y[4]
    spin2 <- CL*(vw[3]*(y[2] - W[1]) - vw[1]*(y[6] - W[3]))/w
    if(is.na(spin2))
      spin2 <- 0
    #dy4 <- coeff*(CD*(y[4] - W[2]) - CL*(vw[3]*(y[2] - W[1]) - vw[1]*(y[6] - W[3]))/w)
    dy4 <- coeff*CD*(y[4] - W[2]) - spin2
    dy5 <- y[6]
    spin3 <- coeff*CL*(vw[1]*(y[4] - W[2]) - vw[2]*(y[2] - W[1]))/w
    if(is.na(spin3))
      spin3 <- 0
    #dy6 <- coeff*(CD*(y[6] - W[3]) - CL*(vw[1]*(y[4] - W[2]) - vw[2]*(y[2] - W[1]))/w) - g
    if(y[5] <= 0 & t != 0){
...
# To state that when the ball hits the ground motion stops
